...
Service.prototype.registerPermission = function (permissionData, callback) {
    if (!permissionData || !permissionData.permissionID || !permissionData.name) {
        callback(null, null);
        return;
    }
    var permission = {};
    permission.permissionID = permissionData.permissionID;
    permission.permissionName = permissionData.name;
...
Service.prototype.obtainAllPermission = function (callback) {
    this.__permissionRepository__.getAllPermission(function (err, permissionDatas) {
        if (err) {
            callback(err, null);
            return;
        }
...
Service.prototype.assignPermissionsToRole = function (permissionIDs, roleID, callback) {
    ...
    async.waterfall([
        ...
        function (permissions, cb) {
            ...
            IDs = [];
            for(var ind in permissions){
                IDs.push(permissions[ind].permissionID);
            }
...
Service.prototype.obtainAllRole = function (callback) {
    this.__roleRepository__.getAllRole(function (err, roles) {
        ...
        for (var ind in roles) {
            roleData = {};
            roleData.roleID = roles[ind].rolesID;
            roleData.roleName = roles[ind].rolesInfo.name;
